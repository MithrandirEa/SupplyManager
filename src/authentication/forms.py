...
class CustomUserCreationForm(UserCreationForm):
    def __init__(self, *args, **kwargs):
        ...
        # Si l'utilisateur connecté n'est pas Admin, retirer l'option Admin des choix
    ...
    def clean(self):
        ...
        if role == User.CREW:
            if not date_start_contract:
                self.add_error('date_start_contract', 'La date de début de contrat est obligatoire pour les membres de l\'\u00e9quipe.')
            if not date_end_contract:
                self.add_error('date_end_contract', 'La date de fin de contrat est obligatoire pour les membres de l\'\u00e9quipe.')
